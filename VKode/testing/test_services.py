...
class ServicesTest(TestCase):
    ...
    def test_count_qr_code_hash(self):
        ...
        expected_result = hashlib.sha3_256(
            f"{self.user.username} {self.qr_code.code_name}".encode()
        ).hexdigest()
    ...
    def test_get_transitions_by_code(self):
        Transition.objects.create(
            code=self.qr_code, user_agent="Mozilla/5.0", ip_address="127.0.0.1"
        )
    ...
    def test_get_plot_html(self):
        data = pd.DataFrame(
            {"minute_of_transition": [datetime.now(timezone.utc)], "users": [1]}
        )
