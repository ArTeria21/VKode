...
class Category(UUIDMixin, CreatedMixin, ModifiedMixin):
    ...
    category = models.CharField(
        verbose_name="Название категории", max_length=255, null=False, blank=False
    )
...
class QRCode(UUIDMixin, CreatedMixin, ModifiedMixin):
    ...
    code_name = models.CharField(
        verbose_name="Название QR кода", max_length=255, null=False, blank=False
    )
    ...
    direction = models.URLField(
        verbose_name="URL адрес, куда ведёт QR код", null=False, blank=False
    )
    ...
    path_to_file = models.FilePathField(
        verbose_name="Путь до QR Кода", editable=False, blank=False, null=False
    )
...
class Transition(UUIDMixin, CreatedMixin, ModifiedMixin):
    ...
    ip_address = models.GenericIPAddressField(
        verbose_name="IP пользователя, перешедшего по коду", null=True, blank=True
    )
